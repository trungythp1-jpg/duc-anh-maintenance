/*
 * Building Module V1.1 — App Shell
 * Derived from Buildings V1.26 current business logic.
 * Only the shell/navigation is changed; Firebase/business logic is retained.
 */
...
export async function mountBuildingModule(container) {
  if (!container) throw new Error("BUILDING_MODULE_CONTAINER_REQUIRED");
  injectStyle();
  container.innerHTML = BUILDING_HTML;
...
document.querySelectorAll(".tab").forEach(button => {
  ...
  return () => {
    ...
    if (typeof window.__openBuildingModal === "function") delete window.__openBuildingModal;
    container.innerHTML = "";
  };
}
