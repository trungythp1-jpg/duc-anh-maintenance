...
function sortBySequence(a, b){
  const aCode = String(a?.assetCode || "").trim();
  const bCode = String(b?.assetCode || "").trim();
  const aMatch = aCode.match(/(\d+)\s*$/);
  const bMatch = bCode.match(/(\d+)\s*$/);
...
function render(){
  const list = filtered().sort(sortBySequence);
  ...
  $("tbody").innerHTML = pageList.map(elevator => `
    <tr>
      <td>
  ...
  $("mobileList").innerHTML = pageList.map(elevator => `
...
function renderPagination(totalPages){
  ...
  let html = `<button class="page-button" data-page="${currentPage - 1}" type="button" ${currentPage === 1 ? "disabled" : ""}>‹</button>`;

  for(let page = 1; page <= totalPages; page++){
    html += `<button class="page-button${page === currentPage ? " active" : ""}" data-page="${page}" type="button">${page}</button>`;
  }

  html += `<button class="page-button" data-page="${currentPage + 1}" type="button" ${currentPage === totalPages ? "disabled" : ""}>›</button>`;
