...
export async function createElevator(data) {
  ...
  const result = await runTransaction(db, async transaction => {
    // All reads must happen before writes inside a Firestore transaction.
    ...
    const elevator = {
      ...
      // Mã thang là định danh lâu dài, tự sinh và không nhận từ frontend.
      assetCode,
...
export async function updateElevator(elevatorId, data) {
  ...
  // assetCode is deliberately excluded: the DAE code is immutable.
  // Existing legacy records are preserved as-is until a dedicated migration is run.
